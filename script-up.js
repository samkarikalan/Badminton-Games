...
function AischedulerNextRound(schedulerState) {
  ...
  let selectedPairs = findDisjointPairs(freePlayersThisRound, new Set(), neededFreePairs, opponentMap);
...
function goToRounds() {
  ...
  if (allRounds.length <= 1) {
    ...
    if (changed) {
      ...
      allRounds[allRounds.length - 1] = [AischedulerNextRound(schedulerState)];
...
function updSchedule(roundData, schedulerState) {
  ...
  // 4️⃣ Track pairs played together
  for (const game of games) {
